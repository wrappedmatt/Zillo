# LemonadeApp - Stripe Terminal Android App

This is the Stripe Terminal Android application for the LemonadeApp loyalty management system. The app runs on Stripe S700 DevKit smart readers and provides a point-of-sale experience for accepting payments and managing customer loyalty points.

## Overview

This Android app integrates with the LemonadeApp ASP.NET Core backend to provide:
...
- **LemonadeApp backend** running (see main project README)
- **Stripe account** with Terminal enabled
- **Stripe test mode API key** configured in LemonadeApp.Server

## Setup

### 1. Configure Backend URL

The Android app needs to communicate with your LemonadeApp backend. Edit `local.properties`:

```properties
# For development with ngrok (recommended for physical device)
...
1. Start your LemonadeApp backend:
   ```bash
...
2. Select **File → Open**
3. Navigate to `LemonadeApp/LemonadeTerminalApp`
...
## Integration with LemonadeApp Backend

### Required Backend Endpoints

The terminal app expects the following API endpoints from the LemonadeApp backend:
...
#### Loyalty Management
```
GET  /api/terminal/lookup_customer_credit?payment_intent_id={id}
POST /api/terminal/apply_redemption
POST /api/terminal/capture_with_redemption
POST /api/terminal/email_receipt
```

### Adding Endpoints to LemonadeApp.Server

You'll need to add a `TerminalController` to handle these endpoints. Example:
...
See the original `StripeTerminalBackend` folder in the example code for reference implementation.
...
QR codes link to your customer enrollment page. Update the URL in `QRCodeFragment.kt`:

```kotlin
// Line 36 in QRCodeFragment.kt
val signupUrl = "https://yourdomain.com/signup"
```

The QR code is generated using the ZXing library and displays as a full-screen image for customers to scan.
...
The app uses `com.lemonade.terminal.loyalty` as the application ID. To change this:
...
**Solutions**:
- Verify backend is running: `curl http://localhost:7024`
...
**Solutions**:
- Verify Stripe API key in `LemonadeApp.Server/appsettings.json`
...
Filter Logcat by tag: `LemonadeTerminalApp`
...
For issues with:
- **This terminal app**: Create an issue in the main LemonadeApp repository
- **LemonadeApp backend**: See main project documentation
- **Stripe Terminal**: Contact [Stripe Support](https://support.stripe.com/)

## Next Steps

1. **Add TerminalController** to LemonadeApp.Server with required endpoints
2. **Test payment flow** with Stripe test cards
3. **Integrate loyalty points** with your Customer/Transaction database
4. **Customize branding** to match your business
5. **Deploy to production** S700 devices

## License

This project is based on the [Stripe Terminal Apps on Devices sample](https://github.com/stripe-samples/terminal-apps-on-devices) and adapted for the LemonadeApp loyalty management system.
