...
class InputFragment : Fragment(R.layout.fragment_input), OnTouchListener {
    ...
    private fun initView(view: View) {
        ...
        launchAndRepeatWithViewLifecycle {
            checkoutViewModel.currentPaymentIntent.collect { paymentIntent ->
                android.util.Log.d("InputFragment", "Payment intent status: ${paymentIntent?.status}, hasNavigated: $hasNavigated")

                // Always use redemption flow: navigate when payment is authorized (REQUIRES_CAPTURE)
                // Payment is authorized for max amount before showing redemption screen
                paymentIntent?.takeIf {
                    it.status == PaymentIntentStatus.REQUIRES_CAPTURE && !hasNavigated
                }?.let { pi ->
                    hasNavigated = true
                    android.util.Log.d("InputFragment", "Payment authorized (REQUIRES_CAPTURE), navigating to redemption screen")
                    // Navigate to show redemption options - payment is authorized, ready to capture
                    findNavController().navigate(
                        InputFragmentDirections.actionInputFragmentToRedemptionFragment(
                            amount = pi.amount,
                            customerId = "",
                            creditAvailable = 0f
                        ),
    ...
    private fun requestNewPayment() {
        ...
        val createPaymentParams = CreatePaymentParams(
            amount = inputViewModel.amount.value.toInt(),
            currency = "nzd",
            description = "Apps on Devices sample app transaction",
        )

        // Always use redemption flow with manual capture
        // Note: Navigation is handled by the flow observer when app returns to foreground
        checkoutViewModel.createRedemptionPaymentIntent(
            createPaymentParams,
            onSuccess = { paymentIntent ->
                android.util.Log.d("InputFragment", "Payment method collected via callback - flow observer will handle navigation")
                // Don't navigate here - flow observer handles it when fragment is in foreground
            },
            onFailure = { failureMessage ->
                android.util.Log.e("InputFragment", "Payment collection failed: ${failureMessage.value}")
                Snackbar.make(
                    viewBinding.root,
